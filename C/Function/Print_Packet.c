...
void printf_byte(const char *label, const uint8_t *array, int start_index, int size) {
    if (DEBUG) {
...
void printf_byte_s(const char *label, const uint8_t *array, int start_index, int size) {
    printf("  [%s] [%d]", label, size);
    for (int i = 0; i < size; i++) {
        if (i % 25 == 0)
            printf("\n    ");
